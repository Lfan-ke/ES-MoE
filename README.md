...
- `weights/<run>-last.pt` — the final epoch's EMA weights, for every same-configuration run and for the earlier protocol runs whose backups kept them. The four same-configuration arms are compared on their final weights rather than on `best.pt`, because the B arm's `best` was selected by a validation metric its evaluation path had broken (`main:docs/JUDGMENT.md`, round seven addendum), and `scripts/measure.py` re-measures all four from `last.pt`.
