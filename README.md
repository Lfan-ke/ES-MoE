...
The 137 `best.pt` checkpoints behind the protocol matrix on `main`: seven backbones (YOLOv5n / v8n / v9t / v10n / 11n / 12n / 26n) with the baseline, esmoe and esmoe-rewire arms, the alignment arms that followed (output norm, dense training, four blocks, the gate-reading objectives, no balancing term), and two rounds of the same-configuration comparison against YOLO-Master (mixed precision and FP32), whose 24 runs also publish `last.pt`. Three seeds or more each; VisDrone, imgsz 800, 120 epochs, `patience=0`, batch 32.
...
- `weights/<run>-last.pt` — the final epoch's EMA weights, for the same-configuration runs only. Those four arms are compared on their final weights rather than on `best.pt`, because the B arm's `best` was selected by a validation metric its evaluation path had broken (`main:docs/JUDGMENT.md`, round seven addendum), and `scripts/measure.py` re-measures all four from `last.pt`.
