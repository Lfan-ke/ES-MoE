...
当前版本 **1.1.0**：文档站可以直接试玩这些模型——上传一张图片，最多三个模型在同一张图上并排跑，带 ES-MoE 块的还会逐块列出各专家的分值与真正参与计算的那几个。推理在读者自己的浏览器里完成。公开接口与 1.0 相同。

## 新增

- **试玩页。** 文档站新增[试玩](demo.md)：COCO 组是官方 YOLO11n 与 YOLO-Master 发布的 EsMoE-N，VisDrone 组是同配置对照的三臂。点样例图或上传自己的图片，样例条保留最近五张；每个模型给出检测框、耗时与目标数，带块的再给每个块一组专家分值，被路由选中的高亮，被推理剪枝去掉的按实际情况标注。浏览器支持 WebGPU 时用它，否则回落到 WASM。
- **模型导出脚本。** `scripts/demo_export.py` 把 `checkpoints` 分支的权重导出成 ONNX，并把每个块的路由概率一并作为输出；导出后逐项核对 ONNX 与 PyTorch 前向一致（概率按绝对值、框按相对尺度、类别分数按绝对值），把追踪器烤进图里的零张量换成形状常量（四个块在 800 像素下省下约 17 MB），并写出 `models/index.json`：类名、输入尺寸、每块的专家数、top-k、专家核大小与推理剪枝阈值、来源权重与文件哈希。
- **上游权重的转移。** `scripts/demo_export.py transfer` 在上游分支的环境里把 fork 训练的检查点存成纯张量，官方 ultralytics 随后按本包的配置重建并加载。
...
- **发布与 CI 检查。** 标签与 `esmoe.__version__` 不一致时发布失败；CI 严格构建文档站。
- **文档站按 x.y 分版本。** x.y.0 发布时部署 `x.y` 并在它是最新时移动 `latest`；x.y.z 小版本不单独成版，更新留在开发版 `dev`。现有两份更名为 `1.0` 与 `0.1`。
- **测试。** 新增图表数据、数据集统计、审计声明与版本号一致性的测试，共 335 项。
- **检查点。** `checkpoints` 分支补上 43 次早先协议运行的 `last.pt`，共 72 个；每个都先核对同一运行的 `best.pt` 与记录和分支上的哈希一致。
- **旧版本文档。** 0.1 的文档保留，删去已退役的页面与已被推翻的说法，每页顶部提示它不是最新版本。
