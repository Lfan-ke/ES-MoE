...
The same script also writes `docs/assets/effect.svg` and `docs/assets/alignment.svg`, which README and the wiki use, because GitHub strips scripts in both places and an interactive chart would not render. The second figure carries the alignment arms -- upstream's settings inside the block, and its layout of four blocks -- which compare block configurations on one backbone and do not fit the generation axis above.

## The alignment arms

![Paired deltas for the alignment arms](assets/alignment.svg)

Upstream's two settings inside the block and its four-block layout, each against a baseline of the same seed. Generated by `scripts/charts.py` like everything else here.
