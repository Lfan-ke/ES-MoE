...
- **预测 3（A − A0 与 B − C 都仍判「有效」）未获证实。** A − A0 仍为有效。B − C 三个 seed 都为正，但 seed 0 只有 +0.0042，区间跨零，记「无法判定」。预登记的推翻条件是任一变成「无效」，没有触发。
...
**FP32 噪声底**：五对重复运行跑完并重测后，按预登记用 FP32 量出的均值 / 最大值把上表再判一遍，档位不同时两个都报。
