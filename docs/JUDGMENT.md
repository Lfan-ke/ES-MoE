...
**必须写明的混淆项**：两个目标在同一个 `weight=0.01` 下的绝对尺度不同——均匀分派时 Switch 取 2.0、GShard 取 1.0，因此这一臂承受的平衡压力大约是对照臂的一半。所以本轮能下的结论是「**在 weight=0.01 这一档，读门控没有解除塌缩**」，不是「读门控的目标做不到」。要分清这两件事，需要按项的尺度对齐权重再跑一轮，本轮没有跑。

**预登记的第四条预测不成立，照实记录。** 后续该做的是权重对齐的对照，而不是换目标。
