...
1. **v5n 整代重跑**。原 v5n 六轮跑在另一台曦云上（`metax3.7.0.7` / Python 3.12），新盒是 `metax3.3.0.2` / Python 3.10。跨软件栈配对不合本项目「同一主干同一张卡」的纪律，因此 v5n 在新盒重跑九轮：既补齐缺的 `rewire` 臂，也得到一份同配置换主机的复现对照。`scripts/report.py` 的分组键已加入硬件栈，两台主机的同名配置不再被折进同一格。
