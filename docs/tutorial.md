...
| 目标 | 公式 | 读什么 |
|:--:|:--:|:--:|
| `gshard_balance`（默认） | `N · Σ usageᵢ²` | 平均概率（上游 `ES_MOE` 用的） |
| `switch_balance` | `E · Σ p̄ᵢfᵢ` | 平均概率 × 实际负载 |
| `master_balance` | `(1/E) · Σ(μᵢ − 1/E)²` | 门控后的权重（论文式 13 用的） |
...
上游代码与其论文在这一点上不一致，取舍与实测见[已知局限](limitations.md)与[判读线](JUDGMENT.md)。
