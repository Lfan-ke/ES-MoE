...
块把每张图交给若干卷积专家里的少数几个，再把它们的输出加权求和。专家的核大小不同，感受野也就不同；由一个轻量路由器按图打分，决定用哪几个。负载均衡项防止路由坍塌到单个专家，它本身就是被优化的损失的一部分。
...
每个专家是一路深度可分离卷积，各自带不同的核大小（3、5、7、9……），所以专家之间的差别在**感受野**而不只是权重。轻量路由器按图给专家打分，取 top-k 后重新归一化，块返回它们的加权和。块保持通道数不变，官方 `parse_model` 才能为它定尺寸。
...
- **trainer 会从配置重建模型**，并且 EMA 副本在任何 callback 之前生成。因此辅助损失权重除了挂在实例上，还存进程作用域；一个进程一次只训练一种辅助损失设置。
- **`loss_items` 的形态跨版本变过**：8.4.13x 之前是 `tensor`，之后是具名 `dict`。两种都已处理；自己 patch 损失时，也要两种都处理。
