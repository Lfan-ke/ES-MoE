...
其中 $E$ 是专家数，$\bar{p}_i$ 是专家 $i$ 在该批次上的平均路由概率，$f_i$ 是实际激活它的样本比例。路由质量与实际负载都摊平时该项最小。实测它防住的是专家死掉，防不住分派集中（[判读线](JUDGMENT.md)第六轮）。
...
在第 10 层的位置插一层，所有大于等于 10 的引用就都往前错了一层。模型照样能建、照样能训，但引用已经错开。所以 `graft` 会对每个插入点之后的引用统一重编号，并有单测把改写后的 head 与原 head 逐条引用比对。
...
上游 `ES_MOE` 与本包默认的差别在下表前五行：四项影响训练，剪枝只影响推理。**都从 `equip` 传进去**；与上游同配置对比时，再加上训练方式 `recipe="upstream"`（见 [API](API.md)）：
    ...
    model = esmoe.equip(
        ...
        recipe="upstream",        # 上游训练器对含路由块模型的三项处理
    )
...
- 通道在首次前向时推断：在任何前向之前 script、导出或加载 `state_dict`，块里还没有专家权重可以载入。
- `attach_aux_loss` 按进程保存权重与配方，一个进程一次训练一种辅助损失设置；在从未调用它的进程里加载检查点，训练时不带辅助损失。
- `loss_items` 的形态在 ultralytics 8.4.13x 前后不同，两种都已处理。
- 块在训练与推理中各怎么算，见[ES-MoE 与 YOLO](design.md)；每个数字怎么量出来的，见[实验](experiments.md)。
