...
| 主干每 stage 一块、共四块（上游 YAML） | 本包 `at="backbone_stages"` 复刻该布局后，配对 mAP50 在 v10n 上 **−0.0071**（0/3）、v5n 上 **−0.0049**（0/3），mAP50-95 两个主干也都是 0/3。四块把辅助项按块求和，同权重下总量是单块的四倍；每块权重降到 0.0025、总量对齐单块后，v10n 上是 **−0.0113**（0/3），mAP50-95 区间 [−0.0132, −0.0018] 不跨零。差在块数，不在平衡压力。方向与论文 Table 5「Both 掉 5.9 点」一致：**ES-MoE 放得越多越差**。 |
